...
export function Games() {
  ...
  const { status, games, pagination, error, retry } = useGames({ page, limit: PAGE_SIZE });

  return (
    <div className="games-page container">
      <Link to="/" className="back-link">{t('common.back')}</Link>
      <Reveal><header className="games-page__head">
        <h1>{t('games.title')}</h1>
        <p>{t('games.intro')}</p>
      </header></Reveal>
        ...
        <>
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className="games-page__pagination">
              ...
              <span>{t('games.page')} {pagination.page} / {pagination.totalPages}</span>
              <Button
                variant="secondary"
